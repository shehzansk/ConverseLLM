# How to Configure HTTPS for ConverseLLM AWS private deployment
...
**Requirements**
- Successful deployment of Amazon Linux 2023 EC2 instance with Docker container running ConverseLLM
...
## Step 7: Create simple http proxy configuration for ConverseLLM 
These instructions are for CLI configuration and assume you are logged in to EC2 instance as the ec2-user.
...
3. Navigate to http://FQDN in a browser and you should be proxied to the ConverseLLM web UI.
...
1. $sudo certbot --nginx -d [Insert FQDN here] 
    ...
    configuration file for conversellm located at /etc/nginx/conf.d/anything.llm
...
2. Navigate to https://FQDN in a browser and you should be proxied to the ConverseLLM web UI.
